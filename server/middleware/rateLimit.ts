...
export const authLimiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    res.status(429).json({
      ...
      retryAfter: Math.ceil(config.authRateLimit.windowMs / 1000 / 60),
    });
  },
});
...
export const passwordResetLimiter = rateLimit({
  windowMs: config.passwordResetRateLimit.windowMs,
  max: config.passwordResetRateLimit.max,
  standardHeaders: config.passwordResetRateLimit.standardHeaders,
  legacyHeaders: config.passwordResetRateLimit.legacyHeaders,
  handler: (req, res) => {
    logger.warn({
    ...
    res.status(429).json({
      ...
      retryAfter: Math.ceil(config.passwordResetRateLimit.windowMs / 1000 / 60),
    });
  },
});